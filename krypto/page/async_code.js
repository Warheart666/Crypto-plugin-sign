    ...
    cadesplugin.async_spawn(function* (arg) {
        ...
        try {
            ...
            if (dataToSign) {
                ...
                var chunks = Math.ceil(dataToSign.size / chunkSize);
                var currentChunk = 0;
                ...
                var frOnload = function (e) {
                    ...
                    if (currentChunk < chunks) {
                     ...
                     //   document.getElementById("SignatureTxtBox").innerHTML = "OK" ;

                        // Проверим подпись
                       var verifyResult = oSignedData.Verify(sSignedMessage, oHashedData).then((resp)=>{
                        if (resp) {
                ...
                function loadNext() {
                    ...
                    var start = currentChunk * chunkSize,
                        end = ((start + chunkSize) >= dataToSign.size) ? dataToSign.size : start + chunkSize;

                    fileReader.readAsDataURL(blobSlice.call(dataToSign, start, end));
                };
